-- COPY AND RUN THIS FILE IN THE SUPABASE SQL EDITOR
-- This fixes the infinite recursion issues in channel_members policies
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.channel_members', policy_rec.policyname);
    END LOOP;
END
$$;
...
drop policy if exists "Users can view channel members if they are members" on public.channel_members;
